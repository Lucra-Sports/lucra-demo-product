 ...
 *     description: Get top 10 highest scores for the current day with display names
 *     tags: [Numbers]
 *     responses:
 *       200:
...
export const getLeaderboard = async (
  ...
  try {
    const leaderboardData = await db.getTodayLeaderboard(10);
    ...
    logger.error(`Leaderboard error: ${(error as Error).message}`);
    res.status(500).json({ error: "Internal error" });
  }
};
